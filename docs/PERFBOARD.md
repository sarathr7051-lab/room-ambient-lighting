...
ESP32's USB socket faces the top edge. The drawing shows 27 x 18 holes; the
...
3. **A bare wire or leg laid along pads must not cover the holes.** Lay it
   ...
   come through later. If a leg does meet wire across its hole: heat that
...
1-2 mm above its cone once soldered - that includes the resistor's hairpin
leg at (21,10), both 0.1 uF legs, the long cap leg, the diode's band leg. A
...
| Link, black, insulated, ~3 cm | (16,4) to (23,4) | top side. Underside: (16,4) end bent onto the GND pin at (15,4) |
| Link, green, insulated, ~8 cm | (4,8) to (16,8) | top side, **round the top end of the ESP32**: up column 4 to row 1, across above the headers, down between columns 16 and 17, into (16,8). It cannot cross the header bodies. Underside: (4,8) end bent onto the RX2 pin at (5,8); (16,8) end bent onto the diode's band leg at (17,8), **toward 17, away from the D27 pin at (15,8)** |
| 1N4007 | band end (17,8), plain end (21,8) | **band toward the ESP32**. Underside: plain leg laid down column 21 beside the holes of rows 9, 10, 11 = the **junction**; soldered at rows 8 and 9 at step 4, at 10 and 11 when the resistor and DIN wire arrive; then **cut 1 mm past (21,11)** |
| 470 ohm | (19,10) and (21,10) | **standing upright**: body on end over (19,10), that leg straight down. The other leg bent back over the top of the body and down **at an angle** into (21,10), 5 mm away; it must not touch the bare bottom lead of the resistor. Goes in beside the junction leg and is soldered to it |
| 1000 uF | LONG leg (19,16), striped SHORT leg (21,16) | its natural 5 mm spacing, no bending. Underside: short leg laid across (22,16) to the GND bus at (23,16), soldered there, **cut 1 mm past (23,16)**. The 10-13 mm can then sits over rows 14-18, clear of everything |
...
**4, 8, 9, 11, 12, 14, 15**; GND bus rows **3, 8, 9, 10, 11, 12, 14, 15**.
Every other bus pad is soldered when its leg arrives, so the hole is open
until then. The drawing colours these two groups differently.
...
put an eraser under the board's edge at the ESP32's antenna end so the stack
does not rock on the USB socket. Solder **one pin at each end of each
...
16, beside the hole centres. Solder it at rows 4 and 15 first so it stays put,
then at rows 8, 9, 11, 12, 14 (7 joints in all). Same for column 23: rows 3,
8, 9, 10, 11, 12, 14, 15. Trim the ends. Columns 20 and 22 stay empty apart
...
the strain relief for the bend, and cut it 1 mm past the pin. Solder the
...
solder the two together; solder the green wire at (16,8) too. Lay the plain
leg down column 21 **beside** the holes of rows 9, 10, 11; solder it at
**rows 8 and 9 only**; cut it 1 mm past (21,11). Rows 10 and 11 stay open.
...
**6. Capacitors.** 0.1 uF into (19,2) and (23,2). 1000 uF: LONG leg (19,16),
striped SHORT leg (21,16); underside, lay the short leg across (22,16) to the
GND bus at (23,16), solder it at (21,16) and (23,16), cut it 1 mm past
(23,16). Now trim every straight-through leg, brush the board clean.
...
- Ohms 2000, red on the +5V bus, black on the GND bus: the number climbs and
  then shows `1` - the 1000 uF charging, fine. A small **steady** number is a
  short. Stop.
- Ohms 2000, +5V bus to the junction (column 21, rows 8-11): about **470**
  (447 to 494 is in tolerance).
- Ohms 2000, red on the junction, black on the GND bus: climbs, then `1`.
- Ohms 2000, **red on the RX2 pin stub (5,8), black on the +5V bus**: `1`.
  Swap the probes and it may show a number above ~900 or `1` - both fine,
  that is the diode conducting through the 470. **Near 470 or below in either
  direction** = diode shorted or the green link touching the junction.
...
wires not yet on. Push one end of a scrap wire into the **left header's 6th
socket from the top (RX2) from the top side**; leave the other end free.
Adapter in. DCV 20, black probe resting in the empty GND-bus hole (23,8) or
pinched on the bus wire, red probe on:
- the +5V bus: about **5.0 V**;
- the junction: about **5.0 V** (pulled up through the 470);
- the junction again while you touch the scrap wire's free end to the GND
  bus: it drops to about **0.7 V**. That is the diode doing its job; 9 mA
  through the 470, harmless. Stays at 5 V = diode backwards, or green link not
  reaching the pin.
...
| Symptom | Likely | Fix |
|---|---|---|
| Nothing lights, ESP32 LED off, regulator hot | 3V3-GND or VIN-GND header bridge (step 1a), or VIN link not on the bus | ESP32 out. Ohms (5,3)-(5,4): `1` at once. Ohms (15,3)-(15,4): climbs then `1` (the capacitor); a small **steady** number is the bridge. DCV on the buses with the adapter in = 5 V |
...
| ESP32 boots, strip dark | diode backwards; green link not on RX2 or not on the band leg; DIN wire in the wrong hole; WLED GPIO not 16 | step 8a test with the ESP32 out; ohms from the junction to the DIN wire's strip end = 0; WLED Config > LED Preferences > GPIO = 16 |
